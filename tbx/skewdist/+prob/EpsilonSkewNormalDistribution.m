classdef EpsilonSkewNormalDistribution < prob.ToolboxFittableParametricDistribution
    ...
    methods
        ...
        function obj = EpsilonSkewNormalDistribution(theta, sigma, epsilon)
            ...
            % If the user creates an object using the constructor, then the
            % parameters are fixed and the covariance matrix is zero.

            arguments
                theta (1,1) double {mustBeReal, mustBeFinite} = 0
                sigma (1,1) double {mustBeReal, mustBeFinite, mustBePositive} = 1
                epsilon (1,1) double {mustBeReal, mustBeFinite, mustBeInRange(epsilon, -1, 1, "exclusive")} = 0 %#ok<MUSTINRANGE>
            end
        ...
        function obj = set.Sigma(obj, sigma)
            arguments
                obj
                sigma (1,1) double {mustBeReal, mustBeFinite, mustBePositive}
            end
        ...
        function obj = set.Epsilon(obj, epsilon)
            arguments
                obj
                epsilon (1,1) double {mustBeReal, mustBeFinite, mustBeInRange(epsilon, -1, 1, "exclusive")} %#ok<MUSTINRANGE>
            end
    ...
    methods (Static, Hidden)
        ...
        function obj = fit(x, varargin)
            ...
            % Record the negative log likelihood value and the parameter
            ...
            % the data using for fitting ('data'), the censoring
        ...
        function y = logpdffunc(x, theta, sigma, epsilon)
            ...
            u = (x - theta)/sigma;
            y = f0( u , epsilon ) - log(sigma);
            y(isnan(x)) = NaN;

            % Canonical LOGPDF (ESN(0, 1, epsilon)).
            function y = f0( u, epsilon )
                negIdx = u < 0;
                y(negIdx) = log(1 / sqrt( 2 * pi ))  - u(negIdx).^2 / (2 * (1 + epsilon)^2) ;
                nonNegIdx = u >= 0;
                y(nonNegIdx) = log(1 / sqrt( 2 * pi ))  - u(nonNegIdx).^2 / (2 * (1 - epsilon)^2) ;
                y = reshape(y, size(u));
            end % f0
